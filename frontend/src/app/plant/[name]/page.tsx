...
const PlantDetailPage = () => {
  ...
  const handleNextImage = () => {
    if (plant && plant.bilder.length > 1) {
      setCurrentImageIndex((prevIndex) => (prevIndex + 1) % plant.bilder.length);
    }
  };

  const handlePrevImage = () => {
    if (plant && plant.bilder.length > 1) {
      setCurrentImageIndex((prevIndex) => (prevIndex - 1 + plant.bilder.length) % plant.bilder.length);
    }
  };
  ...
  const imageUrl = plant.bilder?.[currentImageIndex] || 'https://via.placeholder.com/1200x800.png?text=Kein+Bild';

  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        {/* Image Section */}
        <div className="relative aspect-w-4 aspect-h-3 rounded-lg overflow-hidden group">
          <Image
            src={imageUrl}
            alt={`Bild von ${plant.deutscherName} (${currentImageIndex + 1}/${plant.bilder.length})`}
            layout="fill"
            objectFit="cover"
            className="transition-opacity duration-300"
          />
          {plant.bilder && plant.bilder.length > 1 && (
            <>
              <button onClick={handlePrevImage} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black bg-opacity-30 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                &#10094;
              </button>
              <button onClick={handleNextImage} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black bg-opacity-30 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                &#10095;
              </button>
            </>
          )}
