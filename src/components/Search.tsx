  ...
  return (
    <div className="relative" ref={searchRef}>
      <input
        ...
        className="w-full sm:w-64 px-4 py-2 text-gray-900 bg-gray-100 rounded-full focus:outline-none focus:ring-2 focus:ring-brand-green border-4 border-green-500"
      />
      {isFocused && results.length > 0 && (
        <div className="absolute mt-2 w-full sm:w-64 bg-white rounded-lg shadow-xl z-10">
          <ul>
            {results.slice(0, 5).map((plant, index) => (
              <li key={plant.id ?? `${plant.name}-${index}`}>
                <Link
